...
variable "deploy_temporal" {
  type        = bool
  default     = false
  description = "Whether to deploy Temporal infrastructure resources (GCS bucket + service account for postgres-pod)."
}

variable "temporal_gcs_bucket" {
  type        = string
  default     = "temporal-pg-backups"
  description = "Suffix for the Temporal PostgreSQL backup GCS bucket name. Final name: <deployment_name>-<temporal_gcs_bucket>."
}
